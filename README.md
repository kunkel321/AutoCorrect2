...
- The files in the sub folders all use relative folder paths, so you should be able to unzip the AutoCorrect2.zip and put the folder anywhere.  Note however that the desktop.ini in each folder that Windows uses for the custom icon location has to be an absolute path.  Mine is "C:\Users\steve\OneDrive\Documents\GitHub\AutoCorrect2\Resources\Icons\AhkBluePsicon.ico".  You should change that to the path on your own computer.  Similarly, optional icons for the subfolders ("folder-core.ico", "folder-data.ico", etc) are in the Resources\ folder, but they also must be assigned by the end user. 
